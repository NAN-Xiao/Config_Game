...
compile_file_path = "./excels_path.txt"
...
class Sheetinterpreter:
    ...
    def interpreter(self):
        ...
        # 将PB转换成py格式
        try:
            command = "protoc/protoc --proto_path=./ %s --python_out=./" % (proto_path + self._pb_file_name)
            os.system(command)
    ...
    for line in open(compile_file_path, "r+"):
        ...
        for xls_file_path in glob.glob(line):
            ...
            for sheet_name in workbook.sheet_names():
                ...
                try:
                    command = "protoc/protoc --proto_path=./ %s --csharp_out=./csharp/" % (proto_path + sheet_name + '.proto')
                    os.system(command)
                except BaseException as e:
